...
The road to pervasiveness is not paved with gold, there are many challenges that faces the design and implementation of pervasive applications. Some of these challenges are \cite{Schiele2010}:
\begin{enumerate}
...
Content-centric networking (CCN) was first introduced by Van Jacobson et al., in order to tackle current network architecture issues such as availability, security and location independence. The main communication model relies on two main CCN packets namely \verb|interest| and \verb|data| packets. It works as follows, a consumer broadcasts its interest of information to all connected nodes. If an node posses the desired data and received and interest packet, it should respond with a data packet\cite{Jacobson:2009:NNC:1658939.1658941}.

 CCN is  based on the ICN concept, namespace in CCN  is hierarchal, for instance, \verb|/tum.de/connected-mobility/iot| matches the figure \ref{fig:ccn-namespace}. Names do not have to be meaningful or readable, they can include hashes, timestamps, ids, etc. A request matches an NDO if its name is a prefix of any named object, for example,  a request with the name  \verb|/tum.de/connected-mobility/iot| matches an NDO with the name  \verb|/tum.de/connected-mobility/iot/pervasive-computing|. CCN natively support on-path caching with name-based routing, however, off-path can also be supported\cite{6563278}.
\begin{figure}[H]
	\centering
...
\subsection{Node-Red}
Node-Red is an open-source flow-based programming project originally developed by IBM Emerging Technology Services and now a part of the JS foundation. Flow-based programming describes an application as a network of black-box elements that exchange data together.  Node-red is widely used as a visual tool for wiring IoT applications that can be deployed locally, on the cloud or on edge devices such as (Raspberry PI, Android, Arduino). Elements are the building blocks of node-red, each element has its own defined purpose and can be given a certain input which in turn returns an output. Further, these elements can be re-used and  have different visuals which makes them easy to use and more handy to a wider range of users. The function of these elements varies from digital, social  or even physical events that include sensors. The network of elements is called a  \textit{flow}. Flows can be easily serialized into JSON objects and thus simplifying  importing, exporting sharing process. Since node-red is an open-source project, therefore, the community is allowed to create new flows and extend current flows for their own use cases and make them available to the public, they are also allowed to report potential problems and bugs to the contributers. 

Node-red user interface is available through the browser, however, there is an API to import and export flows through node-red engine without using the user interface. Figure \ref{fig:node-red} shows the browser window with node-red UI. On the left side there are the elements which represent the building blocks of the flow, they can be dragged and dropped inside the plane area. Afterwards, elements can be wired together and deployed. In the figure, a simple flow including a time-stamp triggered every five seconds and debug node is deployed. On the right side of the figure, printed time-stamps can be seen in the debug pane as a result of wiring the debug element into our flow and connecting it to a configured time-stamp element.


\begin{figure}[H]
	\centering
	\includegraphics[scale=0.65]{images/node-red.png}
